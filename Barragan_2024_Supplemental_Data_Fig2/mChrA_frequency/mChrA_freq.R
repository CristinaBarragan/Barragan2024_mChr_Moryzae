#####################################################################################
############ Plot mChrA frequency written by Cristina Barragan Feb 2024##############

#data <- read.table("mChrA_prop_freq.txt", header=T)
data <- read.table("mChrA_cov_BAM_normalized.txt", header=T)
data <- read.table("mChrA_freq_riceonly.txt", header=T)
...
data <- data %>%
  mutate(bin = cut(normalized_mChrA_cov, breaks = seq(0, 100, by = 1), include.lowest = TRUE))
data

#oryza only
data <- data %>%
  mutate(bin = cut(mChrA_cov, breaks = seq(0, 100, by = 1), include.lowest = TRUE))
data
...
########### clustering mChrA yes and no
library(mclust)
data <- read.table("mChrA_cov_BAM_normalized.txt", header=T)
data
gmm_result <- Mclust(data$normalized_mChrA_cov, G=2)
plot(gmm_result)
